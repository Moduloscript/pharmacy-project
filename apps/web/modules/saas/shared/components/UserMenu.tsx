...
export function UserMenu({ showUserName }: { showUserName?: boolean }) {
	...
	const onLogout = async () => {
		try {
			...
			document.cookie = "better-auth.session_token=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; SameSite=Lax;";
			document.cookie = "better-auth.session_data=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; SameSite=Lax;";
			document.cookie = "better-auth.dont_remember=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; SameSite=Lax;";
			
			// Try to call the server-side sign-out API (but don't wait for it to succeed)
			fetch('/api/auth/sign-out', {
				...
				// Ignore errors - we've already cleared cookies locally
				console.warn("Server-side sign-out failed, but continuing with local sign-out:", error);
			});
