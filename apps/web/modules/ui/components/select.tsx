...
>(({ className, children, ...props }, ref) => (
	<SelectPrimitive.Trigger
		ref={ref}
		className={cn(
			"flex h-9 w-full items-center justify-between rounded-md border border-input bg-card px-3 py-2 text-base ring-offset-background placeholder:text-foreground/60 focus:outline-hidden focus:ring-1 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50",
			className,
		)}
		{...props}
	>
...
>(({ className, children, ...props }, ref) => {
	// Radix UI Select.Item crashes if value is an empty string.
	// We return null to prevent the crash, effectively filtering out invalid items.
	if (props.value === "") {
		return null;
	}
