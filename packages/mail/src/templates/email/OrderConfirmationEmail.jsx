...
export function OrderConfirmationEmail(props) {
    var _a;
    const fmtAmount = (v) => typeof v === 'number' ? `₦${v.toLocaleString()}` : v ? `₦${v}` : '';
    return (<EmailLayout title={`Order Confirmation - #${props.order_number}`} preview={`Thanks ${props.customer_name || ''}! Your order #${props.order_number} has been received.`}>
      <Section>
        ...
        {props.total_amount != null && (<Text style={{ fontSize: 14, margin: '6px 0 0' }}>
            <strong>Total:</strong> {fmtAmount(props.total_amount)}
          </Text>)}
        {props.delivery_address && (<Text style={{ fontSize: 14, margin: '6px 0 0' }}>
            <strong>Delivery address:</strong> {props.delivery_address}
          </Text>)}

        {((_a = props.order_items) === null || _a === void 0 ? void 0 : _a.length) ? (<>
            <Hr style={{ borderColor: '#E2E8F0', margin: '16px 0' }}/>
            <Text style={{ fontSize: 14, fontWeight: 700, margin: 0 }}>Items</Text>
            {props.order_items.map((it, i) => (<Text key={i} style={{ fontSize: 14, margin: '4px 0 0' }}>
                {it.name} × {it.quantity}
