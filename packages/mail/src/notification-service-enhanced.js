...
import { queueLowStockAlert, addNotificationJob } from '@repo/queue';
import { NotificationPreferenceChecker } from '../../api/src/services/notification-preference-checker';
import { NotificationType as PrismaNotificationType, NotificationChannel as PrismaNotificationChannel, NotificationPriority as PrismaNotificationPriority, NotificationStatus as PrismaNotificationStatus, } from '@prisma/client';
/**
...
export class EnhancedNotificationService {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                for (const channel of channels) {
                    ...
                    const preferenceCheck = yield NotificationPreferenceChecker.checkNotificationAllowed({
                    ...
                    yield addNotificationJob(notificationType, jobData, {
                        attempts: 3,
                        priority: 5
                    });
                    // Record notification sent
                    yield NotificationPreferenceChecker.recordNotificationSent(order.customerId, notificationType);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                for (const channel of channels) {
                    ...
                    if (actualChannel === 'email' && priority !== 'high')
                        break;
                }
            }
            catch (error) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                    ...
                    const notification = yield db.notification.create({
                        data: {
                            ...
                            metadata: {
                                ...
                                estimatedDelivery: estimatedDelivery === null || estimatedDelivery === void 0 ? void 0 : estimatedDelivery.toISOString()
                            }
                        }
                    });
                    // Queue notification
                ...
                if (!emailQueued && customer.user.email && (order.status === 'DISPATCHED' || order.status === 'DELIVERED')) {
                    const emailAllowed = yield NotificationPreferenceChecker.checkNotificationAllowed({
                        customerId: order.customerId,
                        channel: 'email',
                        type: notificationType,
                        priority
                    ...
                    if (emailAllowed.allowed) {
                        ...
                        const emailNotification = yield db.notification.create({
                            data: {
                                customerId: order.customerId,
                                orderId: order.id,
                                type: mapType(notificationType),
                        ...
                        yield addNotificationJob(notificationType, emailJob, { attempts: 3, priority: 5 });
    ...
    sendLowStockAlert(product) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                // Get admin users
                const adminUsers = yield db.user.findMany({
                    where: { role: 'admin' },
    ...
    getOrderConfirmationMessage(channel, order, customer) {
    ...
    getPaymentSuccessMessage(channel, order, payment, customer) {
        if (channel === 'sms' || channel === 'whatsapp') {
    ...
    getDeliveryUpdateMessage(channel, order, eta, customer) {
